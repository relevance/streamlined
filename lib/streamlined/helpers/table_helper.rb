module Streamlined::Helpers::TableHelper
  ...
  def streamlined_table_row_buttons(item)
    if model_ui.table_row_buttons
      "<td>#{link_to_show_model(item)}#{quick_edit_button(item)}#{quick_delete_button(item)}</td>"
    else
      ""
    end
  end
  ...
  def streamlined_filter
    if model_ui.table_filter
  	  "<div><form><label for='streamlined_filter_term'>Filter:</label>  <input type='text' id='streamlined_filter_term'></form></div>"
    else
      ""
    end
  end
